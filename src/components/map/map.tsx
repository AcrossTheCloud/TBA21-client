...
import { fetchData } from 'actions/map/map';
...
class MapView extends React.Component<Props, State> {
  ...
  componentDidMount(): void {
    ...
    const geoLayer = new L.LayerGroup().addTo(map);
    ...
    L.geoJSON(json, {
      ...
      onEachFeature: (feature: Feature<GeometryObject>, layer: L.Layer) => {
        layer.on('click', () => console.log('You clicked a feature, weeeoeeewww'));
      }
    }).addTo(geoLayer);
    ...
    this.map.leafletElement.addControl( new LeafletSearch({ layer: geoLayer }) );
  }
  ...
  checkZoom = (callback?: Function): boolean | void => {
    ...
    if (this.map.leafletElement.getZoom() < 8) {
