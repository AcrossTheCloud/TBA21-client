...
export const deleteUserFavourite =
  ...
  async (dispatch, getState) => {
    ...
    let profileDetails = getState().profile.details || {};
    let profileId = getState().profile.id;
    try {
      dispatch({
...
export const addUserFavourite =
  ...
  async (dispatch, getState) => {
    ...
    try {
      ...
      let favouriteByType = profileDetails.favourites[type] || [];
      ...
      if (!favouriteByType.includes(id)) {
        await API.patch("tba21", "profiles", {
          body: {
            favourites: {
              ...profileDetails.favourites,
              [type]: (profileDetails.favourites[type] || []).concat(id),
            },
          },
        });
      }
